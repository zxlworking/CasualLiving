...
public class FastBlurUtil {
    ...
    public static int BLUR_RADIUS = 128;


    public static void getUrlBitmap(Context context, String url, int viewWidth, int viewHeight, OnCompleteListener listener) {
        DebugUtil.d(TAG, "getUrlBitmap::viewWidth = " + viewWidth + "::viewHeight = " + viewHeight);
        ...
        try {
            ...
            BitmapFactory.decodeFile(destFile.getPath(), options);

            int widthSample = options.outWidth / viewWidth;
            int heightSample = options.outHeight / viewHeight;
            DebugUtil.d(TAG, "getUrlBitmap::widthSample = " + widthSample + "::heightSample = " + heightSample);
            int sample = Math.max(widthSample, heightSample);
//            int sample = Math.max(1, minSample);
            DebugUtil.d(TAG, "getUrlBitmap::sample = " + sample);

            options.inJustDecodeBounds = false;
            options.inSampleSize = sample;
            options.inPreferredConfig = Bitmap.Config.ARGB_8888;

            Bitmap scaledBitmap = BitmapFactory.decodeFile(destFile.getPath(),options);

            blurBitmap = doBlur(scaledBitmap, BLUR_RADIUS, false);
        } catch (Exception e) {
            e.printStackTrace();
        ...
        if(listener != null){
            listener.onComplete(url, blurBitmap);
        }
    }
    ...
    //    把本地图片毛玻璃化
    private static Bitmap toBlur(Bitmap originBitmap, int scaleRatio) {
        //        int scaleRatio = 10;
    ...
    private static Bitmap doBlur(Bitmap sentBitmap, int radius, boolean canReuseInBitmap) {
        ...
        DebugUtil.d(TAG, "doBlur::w = " + w + "::h = " + h);
